...
// Relevant to the encoding of data into strings when saving
// assumes max double six digits, i.e 123456.123456 rounded to 40.
#define MAX_ACSII 40
...
void save_pokedex(Pokedex pokedex, char *filename) {
    // In case of an empty pokedex do nothing as a new pokedex is the same
    // In the case of invalid filename also do nothing
    if (pokedex->head == NULL || is_valid_save_name(filename) == 0) {
        return;
    }
